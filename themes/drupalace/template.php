...
function drupalace_button($variables) {
  ...
  // Return normal button for hidden button
  if (in_array('ctools-jump-menu-hide', $element['#attributes']['class'])) {
...
function drupalace_preprocess_page(&$vars) {
  ...
  if (isset($vars['node'])) {
    ...
    $vars['title_additional'] = '<span class="additional">' . $type . '</span><span class="arrow"></span>';
  ...
  // Add developers link
  $vars['developers'] = t('Created by !drupalace', array('!drupalace' => l('Drupalace team', 'http://drupalace.ru', array('absolute' => TRUE, 'attributes' => array('target' => '_blank')))));
}
...
function drupalace_preprocess_node(&$vars) {
  ...
  // Add statistics if module exists
  ...
  if ($vars['teaser']) {
    ...
    if (module_exists('comment')) {
      if ($vars['comment'] > 0) {
        ...
        if ($comment_count > 0) {
          $link = format_plural($comment_count, '1 comment', '@count comments');
        }
        else {
          $link = t('Add new comment');
        }
        $links[] = '<span class = "comments">' . l($link, "node/$node->nid", array('attributes' => array('title' => $node->title), 'fragment' => 'comments', 'html' => TRUE)) . '</span>';
      }
    }
    $links[] = '<span class = "readmore">' . l(t('Read more'), "node/$node->nid", array('attributes' => array('title' => $node->title))) . '</span>';
    $vars['bottom_right_links'] = theme('item_list', array('items' => $links));
  }
  ...
  if ($vars['page'] && in_array($node->type, $navigation_types)) {
    $vars['previous_node'] = drupalace_previous_node($node);
    $vars['next_node']     = drupalace_next_node($node);
  }
 ...
 */
function drupalace_previous_node($node) {
 ...
 */
function drupalace_next_node($node) {
  $query = db_select('node', 'n');
  $query->fields('n', array('nid', 'title'));
  $query->condition('n.type', $node->type);
  $query->condition('n.created', $node->created, '>');
